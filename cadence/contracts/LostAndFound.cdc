...
pub contract LostAndFound {
    ...
    pub event TicketDeposited(redeemer: String, ticketID: UInt64, type: Type, memo: String?, name: String?, description: String?, thumbnail: String?)
    pub event TicketRedeemed(redeemer: String, ticketID: UInt64, type: Type)
    pub event BinDestroyed(redeemer: String, type: Type)
    pub event ShelfDestroyed(redeemer: String)
    ...
    pub resource Ticket {
        ...
        init (item: @AnyResource, memo: String?, display: MetadataViews.Display?, redeemer: String, flowTokenRepayment: Capability<&FlowToken.Vault{FungibleToken.Receiver}>?) {
        ...
        pub fun withdraw(receiver: Capability) {
            pre {
                // @alias for L&F Alias, check for linked wallet account instead
    ...
    pub resource Shelf {
        ...
        access(self) let redeemer: String
        access(contract) let flowTokenRepayment: Capability<&{FungibleToken.Receiver}>?

        init (redeemer: String, flowTokenRepayment: Capability<&{FungibleToken.Receiver}>?) {
            self.bins <- {}
        ...
        pub fun redeemAll(type: Type, max: Int?, receiver: Capability) {
            pre {
                // @alias check the linked account of the redeemer's alias
                // self.redeemer stores the alias
                // receiver.address needs to be checked if alias is linked to that address
                // for now in emulator, let it pass
    ...
    pub resource ShelfManager {
        access(self) let shelves: @{String: Shelf}

        init() {
            self.shelves <- {}
        }

        access(contract) fun ensureShelf(_ redeemer: String, flowTokenRepayment: Capability<&FlowToken.Vault{FungibleToken.Receiver}>?): &LostAndFound.Shelf {
            if !self.shelves.containsKey(redeemer) {
                let storageBefore = LostAndFound.account.storageUsed
                let shelf <- create Shelf(redeemer: redeemer, flowTokenRepayment: flowTokenRepayment)
                let uuid = shelf.uuid 
                let oldValue <- self.shelves.insert(key: redeemer, <-shelf)
        ...
        pub fun borrowShelf(redeemer: String): &LostAndFound.Shelf? {
            return &self.shelves[redeemer] as &LostAndFound.Shelf?
        }
        ...
        pub fun deleteShelf(_ redeemer: String) {
            let storageBefore = LostAndFound.account.storageUsed
            assert(self.shelves.containsKey(redeemer), message: "shelf does not exist")
            let tmp <- self.shelves[redeemer] <- nil
            let shelf <-! tmp!
            ...
            destroy shelf
            emit ShelfDestroyed(redeemer: redeemer)
        }
    ...
    pub resource Depositor: DepositorPublic {
        ...
        // @alias this needs to be tweaked a bit
        // receiver is an alias string, so check if there is a linked account
        // check capabilities of the linked account
        // otherwise, deposit with the alias string as reference 
        pub fun trySendResource(
            ...
            if cap.check<&{NonFungibleToken.CollectionPublic}>() {
                ...
                // @alias trySendResource for alias should check if there is a linked address
                // otherwise just deposit with an alias
                //self.deposit(redeemer: cap.address, item: <-item, memo: memo, display: display)
                self.deposit(redeemer: "@elonmusk", item: <-item, memo: memo, display: display)
            }
        }
    ...
    pub fun borrowAllTicketsByType(redeemer: String, type: Type): [&LostAndFound.Ticket] {
    ...
    pub fun borrowAllTickets(redeemer: String): [&LostAndFound.Ticket] {
        let manager = LostAndFound.borrowShelfManager()
        let shelf = manager.borrowShelf(redeemer: redeemer)
        if shelf == nil {
            return []
    ...
    pub fun redeemAll(type: Type, max: Int?, receiver: Capability) {
        // @alias TODO redeem logic for alias
        // check if the receiver's address is linked to an address

        // for emulator, set that to "@elonmusk"
        let alias: String = "@elonmusk"
        
        let manager = LostAndFound.borrowShelfManager()
        let shelf = manager.borrowShelf(redeemer: alias)
    ...
    pub fun getRedeemableTypes(_ redeemer: String): [Type] {
        let manager = LostAndFound.borrowShelfManager()
        let shelf = manager.borrowShelf(redeemer: redeemer)
        if shelf == nil {
            return []
    ...
    pub fun trySendResource(
        ...
        if cap.check<&{NonFungibleToken.CollectionPublic}>() {
            ...
            LostAndFound.deposit(redeemer: "@elonmusk", item: <-resource, memo: memo, display: display, storagePayment: storagePayment, flowTokenRepayment: flowTokenRepayment)
        }
    }
